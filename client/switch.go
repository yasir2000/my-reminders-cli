...
// BackendHTTPClient represnets the HTTP client for communicating with the Backend API
...
// NewSwitch constructor reates a new instance of command Switch
...
// Switch acts like a constructor and also analyses the CLI args and executes the given command
...
// checkArgs check if the number of passed in args is greater or equal to min args
func (s Switch) checkArgs(minArgs int) error {
	...
	if len(os.Args)-2 < minArgs {
		...
		return fmt.Errorf(
			"%s expects at least: %d args(s), %d provided",
