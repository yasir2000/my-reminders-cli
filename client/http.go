...
// HTTPClient represents the HTTP client which comminicates with reminders backend API
type HTTPClient struct {
	client     *http.Client
	BackendURI string
}

// NewHTTPClient function creates a new instance of the HTTPClient
...
// Create calls that create API endpoint
...
//Fetch calls that fetch API endpoint
...
func (c HTTPClient) apiCall(method, path string, body interface{}, resCode int) ([]byte, error) {
	...
	if err != nil {
		e := wrapError("could not marshal request", err)
		return nil, e
	}
...
// readBody that reads response body
